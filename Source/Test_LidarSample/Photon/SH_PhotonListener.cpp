...
void SH_PhotonListener::connectReturn(int errorCode, const JString& errorString, const JString& region, const JString& cluster)
{
	if (errorCode == ErrorCode::OK)
	{
		...
		m_pClient->opCreateRoom("LiDARLobby", options);
	}
}
// 방 생성 & 참가 성공
...
void SH_PhotonListener::createRoomReturn(int localPlayerNr, const Common::Hashtable& roomProperties, const Common::Hashtable& playerProperties, int errorCode, const Common::JString& errorString)
{
	if (errorCode == ErrorCode::OK)
	{
		int playersize = playerProperties.getSize();

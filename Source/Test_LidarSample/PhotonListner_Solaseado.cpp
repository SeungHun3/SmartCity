...
{
	TMap<FString, int> RoomMap; // channel, PlayerCount;

	if (b_IsDummy)  // dummy일 경우에만
	{
		...
		for (int i = 0; i < RoomSize; i++)
		{
			...
			//채널만 뽑아서
			int PlayerCount = m_pClient->getRoomList()[i]->getPlayerCount();
			UE_LOG(LogTemp, Log, TEXT("// onRoomListUpdate RoomName :: %s , PlayerCount : %d"), *RoomName, PlayerCount);
		}
		// 채널만 뽑아서 플레이어카운트가져오는것까지 필요
		//m_pView->UpdateRoomList();

		return;
	}
	
	if(b_IsFirstConnect) // 더미가 아닌 메인클라이언트가 첫접속시에
	{
